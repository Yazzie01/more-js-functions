...
let score = 0;  //No need for a parameter for a function as it is a global variable

// now let's write a function that will add two to the current score each time it's called
function scoreBasket(){
    return score += 2; //Same as score = score + 2 - shorter syntax;
}

// we could also have a function for a three-pointer
function scoreThree(){
    return score += 3; //Same as score = score + 3;
}
...
function doubleMyNumber(number){
    return number *= 2; //same as number = number * 2;
}
...
console.log(doubleMyNumber(number));
console.log('number after call to doubleMyNumber', number);
...
function calcChange(amount){
    // calculate number of quarters, subtract from total change
    let quarters = Math.floor(amount / 25); //Math.floor will always round down
    amount = amount - (quarters * 25);

    // calculate the number of dimes, subtract from total
    let dimes = Math.floor(amount / 10);
    amount = amount - (dimes * 10);

    // calculate the number of nickles, subtract from total
    let nickels = Math.floor(amount / 5);
    amount = amount - (nickels * 5);

    // set number of pennies
    let pennies = amount; //The amount of pennies left
